...
describe('TaskList', () => {
  ...
  describe('error checking', () => {
    it('shows an error when createTask fails', async () => {
      const user = userEvent.setup();

      mockedTaskApi.fetchTasks.mockResolvedValue([]);
      mockedTaskApi.createTask.mockRejectedValue(new Error('Enter a valid task'));
      
      render(<TaskList />);

      // Wait for it to load
      await screen.findByText(/no tasks yet. add one above!/i);

      // Add a task
      await user.type(screen.getByLabelText(/task title/i), 'Steal the moon');
      await user.click(screen.getByRole('button', { name: /add task/i }));
      ...
      expect(await screen.findByText(/no tasks yet. add one above!/i));
    });
  })
});